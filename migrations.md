...
- Первичные ключи — `uuid` с `DEFAULT gen_random_uuid()`, генерация на стороне БД.
  В моделях EF ключ помечается `DatabaseGeneratedOption.Identity`.
  Значения версии 4 (случайные), поэтому вставки в индекс идут вразнобой —
  при заметных объёмах стоит вернуться к упорядоченному во времени UUIDv7
